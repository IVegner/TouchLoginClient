...
//      }
  ...
  func application(application: UIApplication, didReceiveRemoteNotification notificationInfo: [NSObject : AnyObject]) {
    let identifier = notificationInfo["aps"]!["identifier"] as! Int
	let temp : TouchViewController = TouchViewController()
	print(identifier)
	temp.authenticateUser(identifier)
//	temp.replyToServer(identifier)
	//temp.replyToServer(notificationInfo["aps"]!["identifier"])
